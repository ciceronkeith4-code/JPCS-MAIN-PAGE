...
import { logout, getAnnouncements, getSemesters, getSubjects } from "../store";
...
function UserDropdown({ user, isAdmin, onLogout, initials }: UserDropdownProps) {
  const [open, setOpen] = useState(false);
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-screen">
        <header className="h-16 bg-white border-b border-slate-200 px-5 lg:px-8 flex items-center justify-between shrink-0 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <UserDropdown user={user} isAdmin={isActualAdmin} onLogout={handleLogout} initials={initials} />
          </div>
        </header>
