...
export function StudentManagementPage() {
  ...
  const handleCreateStudent = async (e: React.FormEvent) => {
    e.preventDefault();
    setCreateError(null);
    if (!createForm.email || !createForm.password || !createForm.fullName || !createForm.studentId) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <Button variant="primary" onClick={() => { setCreateError(null); setCreatedCredentials(null); setCreatingStudent(true); }} className="font-bold shrink-0">
      ...
      <Modal open={creatingStudent} onClose={() => setCreatingStudent(false)} title="Create Student Account" size="md">
        {createdCredentials ? (
          <div className="space-y-4">
            <Alert variant="success" title="Account Created Successfully!">
              Copy the login credentials below and provide them securely to the student.
            </Alert>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-2 font-mono text-xs text-slate-800">
              <p><strong>Email:</strong> {createdCredentials.email}</p>
              <p><strong>Temporary Password:</strong> {createdCredentials.pass}</p>
            </div>
            <p className="text-xs text-slate-500">
              The student will be required to change their temporary password upon their first login.
            </p>
            <div className="flex justify-end pt-2">
              <Button variant="primary" onClick={() => { setCreatedCredentials(null); setCreatingStudent(false); }}>
                Done
              </Button>
            </div>
          </div>
          ...
          <form onSubmit={handleCreateStudent} className="space-y-4">
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">School Email *</label>
              <Input
                type="email"
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 block mb-1">Temporary Password *</label>
              <Input
                type="password"
                value={createForm.password}
                onChange={(e) => setCreateForm({ ...createForm, password: e.target.value })}
                placeholder="At least 8 chars (e.g. Temp#2026)"
                required
                disabled={createLoading}
              />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
              <Button type="button" variant="outline" onClick={() => setCreatingStudent(false)} disabled={createLoading}>
                Cancel
              </Button>
              <Button type="submit" variant="primary" disabled={createLoading} className="font-bold">
...
export function AccountRequestsPage() {
  const [requests, setRequests] = useState<import("../types").AccountRequest[]>([]);
  ...
  // Reject Modal State
  const [rejectingReq, setRejectingReq] = useState<import("../types").AccountRequest | null>(null);
  const [rejectionReason, setRejectionReason] = useState("");
  const [rejectLoading, setRejectLoading] = useState(false);
  const [rejectError, setRejectError] = useState<string | null>(null);

  // Detail Modal State
  const [viewReq, setViewReq] = useState<import("../types").AccountRequest | null>(null);
        ...
        {viewReq && (
          <div className="space-y-3 text-xs">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-2">
              ...
              {viewReq.rejectionReason && <p className="text-rose-700"><strong>Rejection Reason:</strong> {viewReq.rejectionReason}</p>}
              {viewReq.createdUserUid && <p><strong>Created User UID:</strong> <span className="font-mono text-slate-600">{viewReq.createdUserUid}</span></p>}
            </div>
            <div className="flex justify-end pt-2">
