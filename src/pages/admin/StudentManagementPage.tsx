...
import { Card, Button, Modal, Input, Select, PageHeader, SearchInput, EmptyState, Alert } from "../../app/components/ui";
...
export function StudentManagementPage() {
  ...
  const handleOfficerChange = async (userId: string, position: string) => {
    ...
    if (result.success && result.data) {
      ...
      setToast(`Updated officer position for ${targetUser.full_name}.`);
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="grid w-full grid-cols-2 gap-1 rounded-xl border border-slate-200 bg-slate-100 p-1 sm:w-auto">
          <button
            ...
            className={`px-3 py-2 text-xs font-semibold rounded-lg ${officerFilter === "all" ? "bg-white text-slate-900 shadow-2xs" : "text-slate-600"}`}
          >
            All Students ({users.length})
          </button>
          <button
            onClick={() => setOfficerFilter("officers_only")}
            className={`px-3 py-2 text-xs font-semibold rounded-lg ${officerFilter === "officers_only" ? "bg-white text-amber-800 shadow-2xs" : "text-slate-600"}`}
          >
            JPCS Officers ({users.filter((u) => u.officer_position && u.officer_position !== "None").length})
          </button>
        </div>
      </div>
                ...
                {filtered.map((u) => (
                  <tr key={u.id} className="border-b border-slate-100 last:border-0">
                    <td className="px-5 py-3.5">
                      ...
                      <p className="text-xs text-slate-500 font-mono">{u.student_number} · {u.email}</p>
                    </td>
                    <td className="px-4 py-3.5 text-center">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${isUserOnline(u.id) ? "bg-emerald-50 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>
                    ...
                    <td className="px-4 py-3.5">
                      <Select
                        ...
                        options={[
                          { value: "None", label: "Standard Student" },
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <Button variant="ghost" size="xs" onClick={() => setDeleting(u.id)} className="text-red-600">Delete</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
